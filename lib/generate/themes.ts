...
export function renderCss(theme: Theme): string {
  ...
  const band = theme.heroDark ? "dark" : "soft";
...
:root {
  ...
  --${p}-shadow: 0 1px 2px rgba(15, 23, 42, 0.06), 0 10px 30px rgba(15, 23, 42, 0.08);
  --${p}-shadow-lift: 0 2px 4px rgba(15, 23, 42, 0.08), 0 20px 46px rgba(15, 23, 42, 0.14);
}
...
.${p}-hero-status li { font-family: ${mono}; font-size: 0.76rem; font-weight: 600; letter-spacing: 0.08em; color: var(--${p}-primary); }
.${p}-hero-status li::before { content: "▪ "; color: var(--${p}-accent); }

/* Hero stage — motif-specific treatment of the image panel */
.${p}-hero-stage { position: relative; display: grid; gap: 1rem; }
...
.${p}-hero-ticket {
  margin: -1.6rem 1.1rem 0; position: relative; z-index: 2; padding: 1rem 1.15rem;
...
.${p}-section-dark h2, .${p}-section-dark h3 { color: var(--${p}-dark-text); }
.${p}-section-dark p { color: var(--${p}-dark-muted); }
...
.${p}-alert {
  ...
  border-radius: ${r.card}px; background: color-mix(in srgb, var(--${p}-accent) 10%, transparent);
  border: 1px solid color-mix(in srgb, var(--${p}-accent) 40%, transparent); box-shadow: var(--${p}-shadow);
}
.${p}-alert strong { display: block; font-family: ${display}; font-size: 1.25rem; color: var(--${p}-dark-text); }
.${p}-alert p { margin: 0.2rem 0 0; color: var(--${p}-dark-muted); }
.${p}-alert-inline {
...
.${p}-media-card-num { position: absolute; top: 12px; left: 12px; padding: 0.3rem 0.55rem; border-radius: 6px; background: var(--${p}-primary); color: #fff; font-family: ${mono}; font-size: 0.76rem; font-weight: 600; letter-spacing: 0.08em; }
...
.${p}-chip:hover { background: var(--${p}-primary); border-color: var(--${p}-primary); color: #fff; }
...
.${p}-rail-num { display: inline-flex; align-items: center; justify-content: center; width: 46px; height: 46px; border-radius: ${r.chip === 999 ? "50%" : 8}px; background: var(--${p}-primary); color: #fff; font-family: ${mono}; font-size: 0.9rem; font-weight: 600; }
